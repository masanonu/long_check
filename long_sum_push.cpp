...
void long_pop::setValue(long value1)
{
  ...
  if (this->value01 > 99999999999999999) {
    std::cout << "The input value is scale out." << std::endl;
    this->setJudge(false);
  } else if (this->value01 < -99999999999999999) {
    std::cout << "The input value is scale out." << std::endl;
    this->setJudge(false);
  } else {
...
public:
  ...
  bool getJudge(void);
  long getSum(void);
  long getSumFirst(void);
  long getResult(void);
  void show(void);
  ~long_sum_push(void);
};
long_sum_push::~long_sum_push(void)
{
  this->value01 = (long)NULL;
  this->value001 = (long)NULL;
  this->sum = (long)NULL;
  this->sum_first = (long)NULL;
  this->result = (long)NULL;
  this->Judge = (bool)NULL;
}
void long_sum_push::setJudge(bool judge)
{
  this->Judge = judge;
}
bool long_sum_push::getJudge(void)
{
  return this->Judge;
}
long long_sum_push::getInput(void)
{
  return this->value01;
}
long long_sum_push::getSum(void)
{
  return this->sum;
}
long long_sum_push::getSumFirst(void)
{
  return this->sum_first;
}
long long_sum_push::getResult(void)
{
  return this->result;
}
void long_sum_push::setValue(long value1)
{
  this->value01 = 0;
  this->value001 = 0;
  this->sum_first = 0;
  ...
  if (this->getJudge() != false) {
    object01.setValue(this->value01);
    this->sum = object01.getSum();
    object02.setValue(this->sum);
    this->sum_first = object02.getFirst();
    this->value001 = this->value01 * 10;
    this->result = this->value001 + this->sum_first;
  }
}
void long_sum_push::space(long value)
{
  if (value < 10) {
    std::cout << "                      *" << std::endl;
  } else if (value < 100) {
...
void long_sum_push::show(void)
{
  if (this->getJudge() != false) {
    ...
    std::cout << "*    sum    : " << this->getSum();
    this->space(this->getSum());
    std::cout << "*  sumFirst : " << this->getSumFirst();
    this->space(this->getSumFirst());
    std::cout << "*   result  : " << this->getResult();
    this->space(this->getResult());
    std::cout << "**************************************" << std::endl;
  } else {
...
{
  std::cout << "This program calculate a push sum first value at long value." << std::endl;
...
int main(void)
{
  ...
  consultant lady;
  lady.msg01();
  if (std::cin >> value01) {} else {
    lady.msg07();
    goto FORMAT;
  }
  lady.setValue(value01);
  lady.show();
 FORMAT:
  value01 = (long)NULL;
  return 0;
